...
| Narrow read rules | ✅ done | **Done (prod, 16–17 Jul 2026):** every collection that was `signedIn()`-readable is now scoped. `jobs` IDOR closed; `users` split into private doc + `publicProfiles` mirror; `companyTags` locked (claim via `claimSeatTag` callable); `companies` scoped to owner/member/admin; `agencies` + `agencyLinks` locked, with an `agencyPanels/{agencyId}` projection (no PII) for dispatch, `agencyBillingEmail` denormalised onto Property + Job, and a `findAgencyByCode` callable for join codes. Mirror fns: `mirrorPublicProfile`, `mirrorAgencyPanel`. Backfills: `backfillPublicProfiles`, `backfillAgencyData`. All verified on staging + prod by REST. |
